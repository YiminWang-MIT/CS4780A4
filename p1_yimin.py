...
def svm(name, clist, d):
  ...
  for digit in range(10):
    ...
    for c in clist:
      ...
      tmpresult=cal_acc(path+valfile,[path+classfile],[1])
      acc=float(tmpresult[0])/tmpresult[1]
      if acc>bestacc: 
        optclist[digit]=c
        bestacc=acc
    classlist.append(path+"classifier/"+name+"_%d_%d_%f.class"%(digit,d,optclist[digit]))
  print "Optimal C for each Digit:"
  print optclist
  testfile=name+".test"
  tot=0
  cor=0
  for digit in range(10):
    path=name+"/"
    valfile=name+"%d.val"%digit
    valresult=cal_acc(path+valfile,[classlist[digit]],[1])
    tot+=valresult[1]
    cor+=valresult[0]
  print "Validation data Acc=%f"%(float(cor)/tot)
...
def cal_acc(test,clas,typ):
  ...
  return [cor,len(truth)]

clist=[0.0001,0.0005,0.001,0.005,0.01,0.05,0.1]
dlist=[1,2,3,4,5]
for d in dlist:
  print "-------d=%d-------"%d
  result=svm("digits",clist,d)
  print "Test data Acc=%f"%(float(result[0])/result[1])
